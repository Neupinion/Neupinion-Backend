...
class ReprocessedIssueServiceTest {
    ...
    @BeforeEach
    void setUp() {
        reprocessedIssueService = new ReprocessedIssueService(issueRepository, reprocessedIssueRepository);
    }
    ...
    void 특정_날짜의_재가공_이슈를_조회한다() {
        ...
        final ReprocessedIssue issue1 = reprocessedIssueRepository.save(
            ReprocessedIssue.forSave("제목1", "image", Category.ECONOMY, issueId, clock));
        final ReprocessedIssue issue2 = reprocessedIssueRepository.save(
            ReprocessedIssue.forSave("제목2", "image", Category.ECONOMY, issueId, clock));
        final ReprocessedIssue issue3 = reprocessedIssueRepository.save(
            ReprocessedIssue.forSave("제목3", "image", Category.ECONOMY, issueId, clock));
        final ReprocessedIssue issue4 = reprocessedIssueRepository.save(
            ReprocessedIssue.forSave("제목4", "image", Category.ECONOMY, issueId, clock));
        final ReprocessedIssue issue5 = reprocessedIssueRepository.save(
            ReprocessedIssue.forSave("제목5", "image", Category.ECONOMY, issueId, clock));

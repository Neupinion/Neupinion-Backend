...
@RestClientTest(GoogleInfoProvider.class)
class GoogleInfoProviderTest {
    ...
    void fail_request_accessToken() {
        ...
        //when
        //then
        assertThatThrownBy(() -> googleInfoProvider.getAccessToken("wrongCode"))
    ...
    void fail_request_memberInfo_InvalidAccessToken() {
        ...
        //when
        //then
        assertThatThrownBy(() -> googleInfoProvider.getMemberInfo("wrongAccessToken"))
            .isInstanceOf(OAuthException.InvalidAccessTokenException.class);
    }

    @DisplayName("accessToken을 요청할 때 구글 서버 에러가 발생하면 예외를 던진다.")
    @Test
